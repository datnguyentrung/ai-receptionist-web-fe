    ...
    <div className={styles.page}>
      ...
      {isRefreshing ? (
        <div className={styles.refreshNotice} role="status">
          Ðang c?p nh?t d? li?u ch?m công...
        </div>
      ) : null}
...
function StudentAttendanceReportsContent() {
  ...
  const deletePreviewItems = useMemo(() => {
    ...
    return deleteTargetIds.map((attendanceId) => {
      ...
      return {
        attendanceId,
        studentName: base?.studentName ?? "H?c viên không xác d?nh",
  ...
  return (
    <div className={styles.page}>
      ...
      <ConfirmModal
        ...
        description="B?n s?p c?p nh?t tr?ng thái di?m danh và dánh giá cho các h?c viên dã ch?nh s?a."
        confirmText="Luu"
        loadingText="Ðang luu..."
        isLoading={isSaving}
        successToastMessage="Luu di?m danh thành công"
      ...
      <ConfirmModal
        ...
        title={
          deleteTargetIds.length > 1 ? "Xóa di?m danh dã ch?n" : "Xóa di?m danh"
        }
        description={
          deleteTargetIds.length > 1
            ? `B?n s?p xóa ${deleteTargetIds.length} b?n ghi di?m danh. Thao tác này không th? hoàn tác.`
            : "B?n s?p xóa b?n ghi di?m danh này. Thao tác này không th? hoàn tác."
        }
        confirmText="Xóa"
        loadingText="Ðang xóa..."
        isLoading={isDeleting}
        successToastMessage="Xóa di?m danh thành công"
        errorToastMessage="Xóa di?m danh th?t b?i"
        onCancel={() => {
          if (isDeleting) {
            return;
          }
            ...
            <ul className={styles.deletePreviewList}>
              {deletePreviewItems.map((item) => (
                ...
                >
                  ...
                  <div className={styles.deletePreviewMeta}>
                    <span>
                      Ði?m danh: {AttendanceStatusLabel[item.attendanceStatus]}
                    </span>
                    <span>
                      Ðánh giá:{" "}
